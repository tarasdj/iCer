var selected_sharing_theme = jQuery('[name="socialloginandsocialshare_share_selected_share_interface"]');
...
window.onload = function () {
    jQuery("#edit-socialloginandsocialshare-share-horizontal-images-0,#edit-socialloginandsocialshare-share-horizontal-images-1, #edit-socialloginandsocialshare-share-horizontal-images-10").click(function () {
    ...
    sharingproviderlist();
    counterproviderlist();
    jQuery("#socialloginandsocialshare_share_rearrange_providers, #socialshare_vertical_rearrange_providers").sortable({
        revert: true
    });
    if (selected_sharing_theme)
        loginRadiusToggleShareTheme(selected_sharing_theme.val());
    jQuery(".socialloginandsocialshare_share_rearrange_providers, .socialshare_vertical_rearrange_providers").find("li").unwrap();
    jQuery("#socialloginandsocialshare_share_veritical").click(function () {
        loginRadiusToggleShareTheme("vertical");
    });
    jQuery("#socialloginandsocialshare_share_horizontal").click(function () {
...
function loginRadiusToggleShareTheme(theme) {
    ...
    var socialloginandsocialshare_share_horizontal = jQuery("#socialloginandsocialshare_share_horizontal");
    var socialloginandsocialshare_share_veritical = jQuery("#socialloginandsocialshare_share_veritical");
    ...
    if (theme == "vertical") {
        ...
        socialloginandsocialshare_share_horizontal.removeClass("active");
        socialloginandsocialshare_share_veritical.addClass("active");
        vertical_position.removeClass("element-invisible");
    } else {
        socialloginandsocialshare_share_horizontal.addClass("active");
        socialloginandsocialshare_share_veritical.removeClass("active");
...
function sharingproviderlist() {
    ...
    if (div && div_vertical) {
        ...
        jQuery('input[name^="socialloginandsocialshare_share_rearrange_providers_list"]').each(function () {
...
function loginRadiusRearrangeProviderList(elem) {
    var ul = jQuery('#socialloginandsocialshare_share_rearrange_providers');
    var input = jQuery('#socialloginandsocialshare_share_chnage_name');
    if (elem.checked) {
        var provider = jQuery("<li id='edit-lrshare-iconsprite32" + elem.value + "' title='" + elem.value + "' class='lrshare_iconsprite32 lrshare_" + elem.value + "'><input type='hidden' value='" + elem.value + "' name='socialloginandsocialshare_share_rearrange_providers_list[]' id='input-lrshare-" + elem.value + "'></li>");
        ul.append(provider);
    } else {
...
function loginRadiusSharingLimit(elem) {
    var checkCount = jQuery('input[name^="socialloginandsocialshare_share_rearrange_providers_list"]').length;
    if (elem.checked) {
...
function toggle_sharing_counter(is_social_share, is_social_counter) {
    ...
    jQuery("#socialshare_providers_list, #rearrange_sharing_text, #socialloginandsocialshare_share_rearrange_providers")[simple_counter]("element-invisible");
...
function toggle_horizontal_widget(is_horizontal) {
    ...
    jQuery("#socialloginandsocialshare_share_show_horizotal_widget, .form-item.form-type-textarea.form-item-socialshare-show-exceptpages, .form-item.form-type-radios.form-item-socialshare-show-pages, #horizontal_sharing_show, .form-item.form-type-radios.form-item-socialloginandsocialshare-share-enable-horizontal-share, .form-item.form-type-select.form-item-socialshare-top-weight, .form-item.form-type-select.form-item-socialshare-bottom-weight, .form-item.form-type-radios.form-item-socialshare-horizontal-location, .form-item.form-type-textfield.form-item-socialshare-label-string")[horizontal_sharing]("element-invisible");
    jQuery("#socialloginandsocialshare_share_show_veritcal_widget, .form-item.form-type-radios.form-item-socialshare-vertical-show-pages, .form-item.form-type-textarea.form-item-socialshare-vertical-show-exceptpages, .form-item.form-type-radios.form-item-socialloginandsocialshare-share-enable-vertical-share, .form-item.form-type-radios.form-item-socialshare-vertical-location")[vertical_sharing]("element-invisible");
}
/*
